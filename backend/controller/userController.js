...
exports.updateAccountDetails = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({
        message: "Invalid userId. It must be a non-empty string or number.",
      });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error handling account details",
        error: error.message,
      });
  }
};
...
exports.createTransaction = async (req, res) => {
  ...
  if (!amount || !createdAt || !createdBy || !imageFile) {
    return res
      .status(400)
      .json({
        message: "Amount, createdAt, createdBy, and image are required.",
      });
  }
...
exports.createTransactionById = async (req, res) => {
  ...
  if (!amount || !createdAt || !createdBy || !websiteName || !id) {
    return res.status(400).json({
      message:
        "Amount, createdAt, createdBy, websiteName, and ID are required.",
    });
  }
...
exports.createId = async (req, res) => {
  ...
  if (!websiteName || !websiteUrl || !username || !imgUrl || !createdBy) {
    return res
      .status(400)
      .json({ message: "All required fields are provided." });
  }
  ...
  try {
    ...
    if (userBalance < 100) {
      return res.status(400).json({
        message:
          "Insufficient balance. Minimum balance of ₹100 required to create ID.",
...
exports.createIdRequest = async (req, res) => {
  const { websiteName, websiteUrl, username, imgUrl, createdBy, status } =
    req.body;
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error creating ID request", error: error.message });
  }
};
...
exports.closeId = async (req, res) => {
  ...
  try {
    ...
    if (idDoc.createdBy !== createdBy) {
      return res
        .status(403)
        .json({ message: "You are not authorized to close this ID." });
    }
    ...
    res
      .status(500)
      .json({
        message: "Error creating close ID request",
        error: error.message,
      });
  }
};
...
exports.requestPasswordChange = async (req, res) => {
  ...
  if (!id || !createdBy || !newPassword) {
    return res
      .status(400)
      .json({ message: "ID, createdBy, and newPassword are required." });
  }
  ...
  try {
    ...
    if (idDoc.createdBy !== createdBy) {
      return res
        .status(403)
        .json({
          message: "You are not authorized to change password for this ID.",
        });
    }
    ...
    res.status(201).json({
      ...
      request: {
        id: passwordChangeRequest._id,
        ...passwordChangeRequest.toObject(),
      },
    });
  } catch (error) {
...
exports.changeIdPassword = async (req, res) => {
  ...
  try {
    ...
    if (selectedItemDoc.createdBy != userId) {
      return res
        .status(403)
        .json({ message: "Unauthorized to change this password" });
    }
...
exports.getAccountDetailsDeposit = async (req, res) => {
  try {
    ...
    if (!userAccount) {
      userAccount =
        (await AdminAccount.findOne({ userId: "1" })) ||
...
exports.createNewDepositTransaction = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Missing required fields: amount, coinsToReceive, websiteName, username, id, createdBy",
      });
    }
