    ...
    try {
      ...
      if (isAdmin) {
        const queryParam = isSuperAdmin && filterId && filterId !== 'all'
          ...
          : '';
        response = await fetch(`${url}/api/admin/admin-transaction${queryParam}`, {
          headers: {
    ...
    <div className={styles.transactionHistory}>
            ...
            >
              ...
              <div className={`${styles.column} ${styles.status} ${styles[txn.status] || styles.defaultStatus}`}>
                <strong>Status:</strong> {
                  isAdmin && txn.status === 'Accepted' && (txn.transactionType === 'withdrawal' || txn.transactionType === 'deposit') 
                    ? 'Completed' 
                    : txn.status
                }
              </div>
      ...
      {selectedImage && (
        <div className={styles.modal} onClick={closeModal}>
          <img src={selectedImage} alt="Transaction Receipt" className={styles.fullImage} />
        </div>
      )}
