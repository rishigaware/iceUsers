...
export default function IdDepositPopup ({ onClose, walletBalance = 0, setWalletBalance , selectedId }) {
  ...
  const handleSubmit = async (event) => {
    ...
    formData.append("createdAtSelectedId", selectedId.createdAt._seconds); // Example: Sending createdAt from selectedId
    formData.append("id", selectedId.id); // Append the unique ID of selected object
  
    try {
      const response = await fetch(
        `${url}/api/user/create-transaction-id`,
        {
          method: "POST",
